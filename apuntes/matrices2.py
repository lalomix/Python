...
# Creamos una matriz de 5x8 donde cada posición contiene una lista con 6 elementos
matriz = [
...
# Definimos el valor a reemplazar y el nuevo valor
valor_a_reemplazar = 120
nuevo_valor = 999
...
for i in range(len(matriz)):
    for j in range(len(matriz[i])):
        for k in range(len(matriz[i][j])):
            if matriz[i][j][k] == valor_a_reemplazar:
                matriz[i][j][k] = nuevo_valor
